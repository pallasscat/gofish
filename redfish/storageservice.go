...
func (storageservice *StorageService) Drives() ([]*Drive, error) {
	return ListReferencedDrives(storageservice.Client, storageservice.drives)
}
...
func (storageservice *StorageService) Endpoints() ([]*Endpoint, error) {
	return ListReferencedEndpoints(storageservice.Client, storageservice.endpoints)
}
...
func (storageservice *StorageService) Redundancy() ([]*Redundancy, error) {
	var result []*Redundancy

	collectionError := common.NewCollectionError()
	for _, redundancyLink := range storageservice.redundancy {
		redundancy, err := GetRedundancy(storageservice.Client, redundancyLink)
		if err != nil {
